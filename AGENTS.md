...
- HostReady waits for `document.body` (poll + DCL, no html observer), then until React host internals appear, with an 8s **ceiling** (not a minimum). After the island gate + idle, auto-start HostReady plugins (`requestChromeReady`). The blossom only toggles the prebuilt settings modal — it must not call `requestChromeReady` or rebuild the modal on click. If the island gate fails, still fire HostShell (FAB) and skip HostReady. ChatStateFavicons must not observe `document.body` with `subtree` (composer-root childList is OK). Do not treat `DOMContentLoaded` or `window` `load` as hydrated for `#bloom-root`. InputHistory must use `StartAt.HostReady` and capture-phase keydown.

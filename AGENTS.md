...
- HostReady waits for `document.body` (poll + DCL, no html observer), then until React host internals appear, with an 8s **ceiling** (not a minimum). If the host is not interactive, do not auto-append `#bloom-root`. Do not remount if React detaches it. GM menu still opens settings. ChatStateFavicons must not observe `document.body` with `subtree`. Do not treat `DOMContentLoaded` or `window` `load` as hydrated. InputHistory must use `StartAt.HostReady`.

...
当前版本：**[v1.4.106](https://github.com/0-V-linuxdo/Bloom/releases/tag/v1.4.106)**（`userscript/Bloom.update3.user.js`，`@version [20260924] v1.4.106`）。
...
v1.4.106：**打开对话不再额外 GET 会话详情。** 消息导航只复用页面自己的窗口化 GET。悬停目录或跳到尚未挂载的消息时才要一页更早的 `num_turns=10`，遇到 HTTP 429 立刻停。空白 assistant 节不再标成 `Message 1`。两条正文相同、chain id 不同的用户消息保持两格。安装地址仍是 `userscript/Bloom.update3.user.js`。

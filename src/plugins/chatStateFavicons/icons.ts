 ...
 * Always a white blossom on a dark plate — the official black favicon disappears
 * against ChatGPT chat backgrounds and translucent browser chrome.
 */
...
/** White blossom on ChatGPT's dark squircle — readable on light tabs and dark wallpapers. */
const MARK = "#ffffff";
const PLATE = "#212121";
const RING = "#212121";
const GLYPH = "#ffffff";
...
export function composeIcon(
    style: IconStyle,
    kind: FaviconKind,
    _officialHref: string,
    _scheme: ColorScheme = "dark",
): string {
    if (style === "original") {
        if (kind === "wait") return toSvgData(plateSvg(MARK, PLATE));
    ...
    if (style === "hole") {
        return toSvgData(plateSvg(color ?? MARK, PLATE));
    }
    if (style === "bg") {
        return toSvgData(plateSvg(MARK, color ?? PLATE));
    }

    if (!color || kind === "wait") {
        return toSvgData(plateSvg(MARK, PLATE));
    }

    const badge = style === "dot"
        ? [
            `<circle cx="52.2" cy="52.2" r="10.4" fill="${RING}"/>`,
            `<circle cx="52.2" cy="52.2" r="7.7" fill="${color}"/>`,
        ].join("")
        : [
            `<circle cx="51.5" cy="51.5" r="12.15" fill="${RING}"/>`,
            `<circle cx="51.5" cy="51.5" r="9.55" fill="${color}"/>`,
            badgeGlyph(kind, GLYPH),
        ].join("");

    return toSvgData(plateSvg(MARK, PLATE) + badge);
}

...
function blossomGroup(fill: string): string {
    return [
        `<g transform="translate(8 8) scale(2)" fill="${fill}" fill-rule="evenodd">`,
        `<path d="${BLOSSOM_PATH}"/>`,
        "</g>",
    ].join("");
}
...
export function composeIcon(style: IconStyle, kind: FaviconKind, officialHref: string): string {
    ...
    if (style === "hole") {
        return toSvgData(plateSvg(color ?? MARK_FILL, HOLE_IDLE));
    }
    if (style === "bg") {
        return toSvgData(plateSvg(MARK_FILL, color ?? HOLE_IDLE));
    }

    if (!color || kind === "wait") return toSvgData(plateSvg(MARK_FILL, HOLE_IDLE));
    ...
    return toSvgData(plateSvg(MARK_FILL, HOLE_IDLE) + badge);
}
